...
def math(coin_type, cost_electricity, hash, potreb, komm):
    if coin_type == "Bitcoin":
        ...
        url = 'https://whattomine.com/coins/334.json'
        response = requests.get(url)
        json_data = response.json()
        block_reward24 = json_data['block_reward24']
        difficulty = json_data['difficulty']
        exchange_rate_json = json_data['exchange_rate_vol'] / 30
        rate = 1e12


    elif coin_type == "Decred":
        url = 'https://whattomine.com/coins/152.json'
        response = requests.get(url)
        json_data = response.json()
        block_reward24 = json_data['block_reward24']
        difficulty = json_data['difficulty']
    ...
    if coin_type == "Bitcoin":
        ...
        profit_coin = lambda time: (time * R * H) / (D * pow(2, 32)) * 0.75
        income = lambda incoin: exchange_rate * float(incoin) * 149
        expense = lambda time, exp: (float(potreb) / 1000 * time * float(cost_electricity) + (float(exp) *
                                                                                              ((float(
                                                                                                  komm) / 100)))) * 1.2
        profit = lambda inc, expen: inc - expen
        ...
        profit_coin = lambda time: (time * R * H) / (D * 1.08)
        income = lambda incoin: exchange_rate * float(incoin)
        expense = lambda time, exp: (float(potreb) / 1000 * time * float(cost_electricity) + (float(exp) *
                                                                                              ((float(
                                                                                                  komm) / 100))))
